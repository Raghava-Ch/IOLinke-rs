...
int main() {
    printf("IOLink C Example\n");
    
    // Create a new device instance
    iolinke_device_handle_t device = io_linke_device_create();
    printf("device handle %d", device);
    if (device < 0) {
    ...
    // Poll the device (example)
    device_action_state_t state = iolinke_device_poll(device);

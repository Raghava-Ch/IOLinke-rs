...
use iolinke_types::handlers::mode::DlModeInd;
use iolinke_types::{
    custom::{IoLinkError, IoLinkResult},
    frame,
    handlers::{self, mode::DlMode},
};
use iolinke_util::{log_state_transition, log_state_transition_error};

use crate::{
    dl::{command_handler, event_handler, isdu_handler, message_handler, od_handler, pd_handler},
...
enum DlModeEvent {
    ...
    /// Mode change request
    /// Table 45 – T2
    ComChange(frame::msequence::TransmissionRate),
    /// Table 45 – T3
...
pub struct DlModeHandler {
    ...
    current_mode: frame::msequence::TransmissionRate,
    target_mode: frame::msequence::TransmissionRate,
}
...
impl DlModeHandler {
    ...
    pub fn new() -> Self {
        Self {
            ...
            current_mode: frame::msequence::TransmissionRate::default(),
            target_mode: frame::msequence::TransmissionRate::default(),
        }
    }
    ...
    pub fn poll(
        ...
        match self.exec_transition {
            ...
            Transition::T4 => {
                self.exec_transition = Transition::Tn;
                let _ = self.execute_t4(pd_handler, system_management, message_handler);
            }
            Transition::T5 => {
                self.exec_transition = Transition::Tn;
    ...
    fn execute_t1(
        ...
        message_handler.mh_conf_update(handlers::message::MhConfState::Active);
    ...
    fn execute_t2(
        ...
        let _ = od_handler.oh_conf(handlers::command::ChConfState::Active);
        // Activate command handler (call CH_Conf_ACTIVE in Figure 54)
        let _ = command_handler.ch_conf(handlers::command::ChConfState::Active);
        ...
        let dl_mode = match self.current_mode {
            frame::msequence::TransmissionRate::Com1 => DlMode::Com1,
            frame::msequence::TransmissionRate::Com2 => DlMode::Com2,
            frame::msequence::TransmissionRate::Com3 => DlMode::Com3,
        };
    ...
    fn execute_t3(
        ...
        let _ = isdu_handler.ih_conf(handlers::isdu::IhConfState::Active);
        // Activate Event handler (call EH_Conf_ACTIVE in Figure 56)
        let _ = event_handler.eh_conf(handlers::event::EhConfState::Active);
        // Indicate state via service DL_Mode.ind (PREOPERATE) to SM
        let _ = system_management.dl_mode_ind(DlMode::PreOperate);
        let _ = message_handler.dl_mode_ind(DlMode::PreOperate);
        Ok(())
    }
    ...
    fn execute_t4(
        ...
        // Activate Process Data handler (call PD_Conf_ACTIVE in Figure 47)
        let _ = pd_handler.pd_conf(handlers::pd::PdConfState::Active);
        // Indicate state via service DL_Mode.ind (OPERATE) to SM
        let _ = system_management.dl_mode_ind(DlMode::Operate);
        let _ = message_handler.dl_mode_ind(DlMode::Operate);
        Ok(())
    }
    ...
    fn execute_t6(
        ...
        let _ = isdu_handler.ih_conf(handlers::isdu::IhConfState::Inactive);
        // Deactivate Event handler (call EH_Conf_INACTIVE in Figure 56)
        let _ = event_handler.eh_conf(handlers::event::EhConfState::Inactive);
        // Indicate state via service DL_Mode.ind (STARTUP) to SM
        let _ = system_management.dl_mode_ind(DlMode::Startup);
        let _ = message_handler.dl_mode_ind(DlMode::Startup);
    ...
    fn execute_t7(
        ...
        // Deactivate Process Data handler (call PD_Conf_INACTIVE in Figure 47)
        let _ = pd_handler.pd_conf(handlers::pd::PdConfState::Inactive);
        // Deactivate ISDU (call IH_Conf_INACTIVE in Figure 52)
        let _ = isdu_handler.ih_conf(handlers::isdu::IhConfState::Inactive);
        // Deactivate Event handler (call EH_Conf_INACTIVE in Figure 56)
        let _ = event_handler.eh_conf(handlers::event::EhConfState::Inactive);
    ...
    fn execute_t8(
        ...
        let _ = isdu_handler.ih_conf(handlers::isdu::IhConfState::Inactive);
        let _ = event_handler.eh_conf(handlers::event::EhConfState::Inactive);
        let _ = command_handler.ch_conf(handlers::command::ChConfState::Inactive);
        let _ = od_handler.oh_conf(handlers::command::ChConfState::Inactive);
        let _ = pd_handler.pd_conf(handlers::pd::PdConfState::Inactive);
        let _ = message_handler.mh_conf_update(handlers::message::MhConfState::Inactive);
        // Indicate state via service DL_Mode.ind (INACTIVE) to SM
        let _ = system_management.dl_mode_ind(DlMode::Inactive);
        let _ = message_handler.dl_mode_ind(DlMode::Inactive);
    ...
    fn execute_t10(
        ...
        self.current_mode = frame::msequence::TransmissionRate::default();
    ...
    pub fn successful_com(&mut self, com_mode: frame::msequence::TransmissionRate) {
...
impl handlers::message::MsgHandlerInfo for DlModeHandler {
    /// See 7.2.2.6 MHInfo
    ...
    fn mh_info(&mut self, mh_info: handlers::message::MHInfo) {
        if mh_info == handlers::message::MHInfo::IllegalMessagetype {
            let _ = self.process_event(DlModeEvent::MHInfoIllegalMessagetype);
        }
    }
}

impl handlers::command::MasterCommandInd for DlModeHandler {
    /// Any MasterCommand received by the Device command handler
    /// (see Table 44 and Figure 54, state "CommandHandler_2")
    fn master_command_ind(
        &mut self,
        master_command: iolinke_types::page::page1::MasterCommand,
    ) -> IoLinkResult<()> {

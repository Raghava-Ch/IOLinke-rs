...
use crate::utils::frame_fromat::isdu;
use crate::*;
use crate::utils::frame_fromat::isdu::IsduService;
...
pub fn util_test_startup_sequence(
    ...
    // Read min cycle time
    let min_cycle_time = page_params::read_min_cycle_time(
        &poll_tx,
        &poll_response_rx,
    ...
    // Read revision id
    let revision_id = page_params::read_revision_id(
        &poll_tx,
        &poll_response_rx,
        TestDeviceMode::Startup,
    );
    ...
    let process_data_in = page_params::read_process_data_in(
        &poll_tx,
        &poll_response_rx,
        TestDeviceMode::Startup,
    );
    ...
    let process_data_out = page_params::read_process_data_out(
        &poll_tx,
        &poll_response_rx,
        TestDeviceMode::Startup,
    );
    ...
    // Read vendor id
    let vendor_id_1 = page_params::read_vendor_id_1(
        &poll_tx,
        &poll_response_rx,
        TestDeviceMode::Startup,
    );
    ...
    // Read vendor id
    let vendor_id_2 = page_params::read_vendor_id_2(
        &poll_tx,
        &poll_response_rx,
        TestDeviceMode::Startup,
    );
...
pub fn util_test_change_operation_mode(
    ...
    let _ = page_params::write_master_command(
        poll_tx,
        poll_response_rx,
        current_mode,
        master_command,
...
pub fn util_test_isdu_sequence_read(
    ...
    let isdu_read_request =
        frame_utils::isdu_frame::create_isdu_read_request(index, subindex);
    println!("isdu_read_request: {:?}", isdu_read_request);

    // Write ISDU request to device
    let mut isdu_frames = Vec::new();
    let mut offset = 0;
    let mut circular_offset = flow_ctrl!(START);
    while offset < isdu_read_request.len() {
        ...
        let chunk = &chunk_buf[..];
        let frame = frame_utils::create_preop_write_isdu_request(
            circular_offset as u8,
            chunk,
        );
        offset += OD_LENGTH_PER_FRAME;
        circular_offset += 1;
        if circular_offset > 15 {
            circular_offset = 0;
        }
        ...
        let is_checksum_valid = frame_utils::validate_device_frame_checksum(
    ...
    loop {
        let isdu_start_frame =
            frame_utils::create_preop_read_start_isdu_request();
    ...
    let is_checksum_valid =
        frame_utils::validate_device_frame_checksum(&mut isdu_idle_response);
    assert!(is_checksum_valid, "Checksum not matching");
...
pub fn util_test_isdu_sequence_write(
    ...
    let isdu_write_request = frame_utils::isdu_frame::create_isdu_write_request(
        index, subindex, data,
    );
    ...
    while offset < isdu_write_request.len() {
        ...
        let chunk = &chunk_buf[..];
        let frame =
            frame_utils::create_preop_write_isdu_request(circular_offset, chunk);
        offset += OD_LENGTH_PER_FRAME;
        // circular_offset wraps around 0..=15
        circular_offset += 1;
        if circular_offset >= 16 {
            circular_offset = 0;
        }
        let mut isdu_write_response = match send_test_message_and_wait(
            &poll_tx,
    ...
    assert!(isdu_service.i_service() == isdu_write_success_code!(), "ISDU data write failed");

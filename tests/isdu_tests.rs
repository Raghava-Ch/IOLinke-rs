use iolinke_device::{
    ...
    config, DataStorageIndexSubIndex, DeviceParametersIndex, SubIndex,
};
...
fn test_isdu_read_vendor_name() {
    ...
    if m_sequence_capability.isdu() {
        ...
        const VENDOR_NAME: &'static str = config::vendor_specifics::vendor_name();
        const VENDOR_NAME_LENGTH: u8 = config::vendor_specifics::vendor_name_length();
...
fn test_isdu_read_product_name() {
    ...
    if m_sequence_capability.isdu() {
        ...
        let product_name_subindex = DeviceParametersIndex::ProductName.subindex(SubIndex::ProductName);
